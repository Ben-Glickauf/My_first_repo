...
ui <- fluidPage(
    ...
    sidebarLayout(
        sidebarPanel(
            ...
            checkboxInput("show_sentence",
                          "Show info about our histogram")
        ),
...
server <- function(input, output, session) {
    ...
    output$hist_info <- renderUI({
      ...
      h4(
        paste0(
          "The bin with the highest number of observations starts at ",
          start_bin,
          " and the number of observations in that bin is ",
          highest_bin
        )
      )
    })
